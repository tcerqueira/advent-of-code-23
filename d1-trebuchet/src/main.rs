...
fn main() {
    // let filepath = env::args().next().expect("you need to provide a filepath as args");
    ...
    for line in input.lines() {
        ...
        for current_char in line.chars() {
            let found_number = match current_char.to_digit(10) {
                Some(num @ 0..=9) => {
                    buffer = "".to_string();
                    Some(num)
                },
                _ => {
                    ...
                    let mut num = None;
                    if buffer.contains("one") { num = Some(1) }
                    if buffer.contains("two") { num = Some(2) }
                    if buffer.contains("three") { num = Some(3) }
                    if buffer.contains("four") { num = Some(4) }
                    if buffer.contains("five") { num = Some(5) }
                    if buffer.contains("six") { num = Some(6) }
                    if buffer.contains("seven") { num = Some(7) }
                    if buffer.contains("eight") { num = Some(8) }
                    if buffer.contains("nine") { num = Some(9) }
        ...
        if first.is_some() && current_num.is_some() {
            counter += first.unwrap() * 10 + current_num.unwrap();
        }
    }
...
const INPUT_EXAMPLE_1: &str = r#"two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
...
const INPUT_EXAMPLE_2: &str = r#"1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet"#;

const INPUT_CONTENT: &str = r#"sdpgz3five4seven6fiveh
876mbxbrntsfm
